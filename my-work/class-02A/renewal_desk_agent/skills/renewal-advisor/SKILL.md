...
description: Evaluates WidgetWare enterprise software contract renewal requests, including discount approval routing, renewal timing and commercial processes, risk escalations, and formatting renewal briefs.
---
...
If required inputs are missing, ask the customer-success manager or user to provide them before proceeding.

## Procedure

1. Identify the question type (e.g., discount band, process timeline, risk level, calculation, or brief generation).
2. Check if the required inputs for that query are provided. If not, request them.
3. Consult the Resource routing map to identify the exact L3 resource path. Load the minimum necessary resource(s) needed for the query. Do not load irrelevant files.
4. If the user explicitly asks to calculate a net ARR or dollar discount amount, run the deterministic script `scripts/calculate_quote.py` using `run_skill_script`. You must pass the script arguments in the `args` parameter as a dictionary containing the keys `"arr"` (the current/list ARR) and `"discount-percent"` (the discount percentage, using a hyphen, NOT an underscore). For example: `{"arr": 92000, "discount-percent": 12}`. Do NOT run this script if the user only asks for the approval path or process timeline without asking for calculation.
5. Retrieve the required facts from the loaded L3 reference file.
6. Generate a response showing concise reasoning, citing the exact relative path of the source file, and differentiating states clearly.
...
- For calculating net ARR or discount amounts: `scripts/calculate_quote.py` (invoke via `run_skill_script` using `"arr"` and `"discount-percent"` keys in the `args` dict)
...
- State that the supplied sources do not establish or support the request.
